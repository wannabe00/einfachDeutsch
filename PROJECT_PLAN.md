...
- [ ] **21.2 Dark-only foundation** — make the app render one dark cinematic theme everywhere: `ThemeContext` pins `dark` (remove the toggle + its localStorage light path), retire `ThemeToggle`/its Sidebar+Settings usages (dead-code sweep), fold the landing's `--brand` accent + Space Grotesk into the in-app token set, set the app canvas/surfaces to the near-black cinematic values. Verify nothing still reads `theme === "light"`. eslint + `tsc` + build clean.
- [ ] **21.3 Navigation shell** — restyle the frame (`Layout`/`TopBar`/`Sidebar`) without changing nav items: **desktop** = spaceship-style top bar carrying the primary nav (logo left, nav center/left, account menu right) + collapsible; **mobile** = a cleaner collapsible left drawer (the current hover-rail pattern replaced). Active-state uses the brand accent; keep `AccountMenu`/`AIPanel`. Responsive both ways.
- [ ] **21.4 Settings v2** — rebuild `SettingsPage` in the dark cinematic language: spaceship-style **vertical tabs + single panel** (7 tabs), row-based controls. Restyle the existing sections (`ProfileSection`, `SecuritySection`, `PreferencesSection`, `DataSection`, `DangerZoneSection`). Richer preferences per the earlier spec: learning tuning, appearance (now just accent, since theme is fixed), language/region (du-Sie + UI language + umlaut bar), privacy & **active sessions** list (unblocked by 20.4).
